...
class TurtlebotNavigation():
    ...
    def get_alpha(self):
        ...
        gate_x, gate_y = self.ballDoor_enemy[0]/100.0-0.30, self.ballDoor_enemy[1]/100.0-0.20# 0.35
        a = 0.24
    ...
    def check_car_bound(self, position, rotation):
        if ( position.y <= -0.2 and rotation < 0.0)or ( position.y >= 1.2 and rotation > 0.0 ) :
            for i in range(3):
                self.move_cmd.linear.x = -0.2
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        x = max(height, width)
        distance = 1.237e-12*pow(x,6) -2.225e-9 *pow(x,5) + 1.576e-6*pow(x,4) -0.0005623*pow(x,3) + 0.1071*pow(x,2) -10.65*x +498.3
        for i in get_center:
